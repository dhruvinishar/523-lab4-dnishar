...
<https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_dnishar>
...
Link to GitHub pages: <https://dhruvinishar.github.io/523-lab4-dnishar/>
...
```{r Question-3-code-chunk}
print("This is the Question 3 code chunk, named as \'Question-3-code-chunk\'")
```
...
```{r Question-4-code-chunk, warning=FALSE}
print("This is the Question 4 code chunk with code chunk option {warning=FALSE} named as 'Question 4 Code chunk'.The code chunk options lets us customize the behavior of the code chunk, here, the 'warning=FALSE' option supresses the warning messsages for the chunk. This code chunk will not print any warning messages because of the code chunk option that is enabled")

warning('This is a warning message that the chunk allows us to print')
```
...
```{r Question-5-code-chunk, eval=TRUE, warning=TRUE, include=TRUE}
print("This is the Question 5 code chunk with code chunk options {eval=TRUE, warning=TRUE, include=TRUE}. The multiple code chunk options defined in this code chunk lets us customize the behavior of the code chunk, here, the 'eval=TRUE' option lets us evaluate the code and include the results as the output. The 'echo=TRUE' option is used to display the code along with its results, 'warning=TRUE' option displays warning messsages for the chunk.")

warning('This is a warning message that the chunk allows us to print because of the option that is enabled')
```
...
1 - **HTML output:**
<https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_dnishar/blob/master/test_markdown.html>

2 - **PDF Output:**
<https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_dnishar/blob/master/test_markdown.pdf>

As we see, there is a notable difference between the two output files.
...
The files that were created can be found here:

1.  **The markdown file:**
    <https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_dnishar/blob/master/test_markdown_updated.Rmd>

2.  **The HTML rendered output:**
    <https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_dnishar/blob/master/test_markdown_updated.html>

**Also pasting the link to the `.pdf` generated as output for
reference:**
<https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_dnishar/blob/master/test_markdown_updated.pdf>
...
The link to the `github_document` is here: https://github.ubc.ca/MDS-2022-23/DSCI_521_lab4_dnishar/blob/master/test_markdown_updated.md

This document is rendered much differently on github compared to any other document output format. The `html_document` output format produces an HTML script file that is then executed by to give a HTML output. It looks like it is rendered as the knitted output of the file. It is rendered best for github and shows us exactly how the visual will look like istead of the raw code.
